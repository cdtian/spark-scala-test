...
object DeequApp {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("run 1 size all Completeness all ApproxCountDistinct all ApproxQuantile 2 Compliance multi Correlation 1 DataType 1 Distinctness 1 Entropy same column").getOrCreate()
